...
const FRANCHISE_BRANDS = ['The Green Solution', 'Native Roots', 'LivWell', 'Star Buds', 'Green Dragon', 'Lightshade', 'Medicine Man', 'Good Chemistry'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, CO ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Colorado dispensary`
      : `${name} ${city} Colorado dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'CO')
...
async function importColoradoDispensaries() {
  console.log('Starting Colorado dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./colorado-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Colorado dispensaries\n`);
