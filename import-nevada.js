...
// Known franchise brands in Nevada
const FRANCHISE_BRANDS = ['Rise', 'Thrive', 'Curaleaf', 'The Dispensary', 'Deep Roots Harvest', 'The Source', 'Zen Leaf', 'Green', 'Planet 13', 'Oasis', 'Jardin', 'Cookies', 'Cultivate', 'Nevada Made Marijuana', 'The Grove', 'Wallflower', 'Silver State Relief', 'Jade Cannabis', 'Greenlight', 'The Sanctuary', 'Mint Cannabis', 'Beyond Hello'
...
async function searchAndEnrichDispensary(dispensaryInfo) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address}, ${city}, NV ${zipCode}`);
  ...
  try {
    ...
    const searchQuery = `${name} ${address} ${city} Nevada dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties
       WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'NV')
...
async function importNevadaDispensaries() {
  ...
  try {
    ...
    console.log(`Found ${data.length} Nevada dispensaries to process\n`);
