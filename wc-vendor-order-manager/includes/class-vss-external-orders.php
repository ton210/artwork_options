...
class VSS_External_Orders {
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table">
                    ...
                    <tr>
                        <th scope="row"><?php esc_html_e( 'Access Token', 'vss' ); ?></th>
                        <td>
                            <input type="password" name="vss_shopify_access_token" value="<?php echo esc_attr( get_option( 'vss_shopify_access_token', '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' ) ); ?>" class="regular-text" />
    ...
    /**
     * Fetches Zakeke files for an imported external order and attaches them to the order items.
     *
     * @param WC_Order $order The newly created WooCommerce order object.
     * @param string   $external_order_id The original order ID from the external platform (e.g., BigCommerce).
     */
    private static function fetch_and_attach_zakeke_files($order, $external_order_id) {
        if ( ! class_exists( 'VSS_Zakeke_API' ) ) {
            $order->add_order_note( __( 'Zakeke API class not available. Cannot fetch customization files.', 'vss' ) );
            return;
        }

        // Use the external order ID to fetch details from Zakeke
        $zakeke_api_response = VSS_Zakeke_API::get_zakeke_order_details_by_wc_order_id($external_order_id);

        if ( ! $zakeke_api_response || ! isset( $zakeke_api_response['items'] ) ) {
            $order->add_order_note( sprintf( __( 'Could not retrieve Zakeke data for external order ID: %s.', 'vss' ), $external_order_id ) );
            return;
        }

        $order_item_updated = false;

        foreach ( $order->get_items() as $item_id => $item ) {
            $zakeke_meta_data_raw = $item->get_meta( 'zakeke_data', true );
            ...
            if ( $primary_zakeke_design_id && ! $item->get_meta( '_vss_zakeke_printing_files_zip_url', true ) ) {
                foreach ( $zakeke_api_response['items'] as $zakeke_api_item ) {
                    if ( isset( $zakeke_api_item['design'] ) && $zakeke_api_item['design'] === $primary_zakeke_design_id ) {
                        if ( ! empty( $zakeke_api_item['printingFilesZip'] ) ) {
                            $item->update_meta_data( '_vss_zakeke_printing_files_zip_url', esc_url_raw( $zakeke_api_item['printingFilesZip'] ) );
                            $order_item_updated = true;
                        }
                        $item->save();
                        break;
                    }
        ...
        if ( $order_item_updated ) {
            $order->add_order_note( __( 'Zakeke print file URLs automatically attached to the imported BigCommerce order.', 'vss' ) );
            $order->save();
        }
    }
    ...
    private static function create_order_from_bigcommerce( $external_order ) {
        ...
        if ( ! is_wp_error( $products_response ) ) {
            ...
            foreach ( $products as $item ) {
                ...
                $new_item_id = $order->add_product( wc_get_product( $product_id ), $item['quantity'] );

                // Check for Zakeke data in product options
                if ( $new_item_id && ! empty( $item['product_options'] ) ) {
                    foreach ( $item['product_options'] as $option ) {
                        // Check for Zakeke data in product options
                        if ( strpos( strtolower( $option['display_name'] ), 'zakeke' ) !== false || 
                             strpos( strtolower( $option['display_name'] ), 'customization' ) !== false ) {

                            // Try to parse as JSON first
                            $zakeke_data = json_decode( $option['display_value'], true );
                            if ( json_last_error() === JSON_ERROR_NONE ) {
                                // If it's valid JSON, store the decoded data
                                wc_add_order_item_meta( $new_item_id, 'zakeke_data', $zakeke_data );
                            } else {
                                // Otherwise, store the raw value
                                wc_add_order_item_meta( $new_item_id, 'zakeke_data', $option['display_value'] );
                            }
                        }
                    }
                }
        ...
        // Fetch and attach Zakeke files using the BigCommerce order ID
        self::fetch_and_attach_zakeke_files( $order, $external_order['id'] );
    ...
    private static function import_shopify_orders() {
        ...
        if ( $response_code !== 200 ) {
            ...
            // Log to fix system if available
            if ( class_exists( 'VSS_Shopify_Fix' ) ) {
                VSS_Shopify_Fix::log_api_error( [
                    'message' => "Shopify API returned status code {$response_code}",
                    'error' => $error_message,
            ...
            return [
                'imported' => 0,
                'message' => sprintf( __( 'API returned status code %d: %s', 'vss' ), $response_code, $error_message ),
                'debug' => $debug_logs
            ];
        }
    ...
    private static function sync_tracking_to_shopify( $order, $tracking_number, $tracking_carrier ) {
        ...
        $access_token = get_option( 'vss_shopify_access_token', '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' );
