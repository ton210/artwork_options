...
trait VSS_Vendor_Orders {
    ...
    private static function render_frontend_orders_list() {
        ...
        $priority_filter = isset( $_GET['priority'] ) ? sanitize_key( $_GET['priority'] ) : '';
        $ship_status = isset( $_GET['ship_status'] ) ? sanitize_key( $_GET['ship_status'] ) : '';

        // Allow vendor to choose how many orders per page
        $per_page_options = [ 25, 50, 100, 250, 500, 1000 ];
        $per_page = isset( $_GET['per_page'] ) ? intval( $_GET['per_page'] ) : 100;
        if ( ! in_array( $per_page, $per_page_options ) ) {
            $per_page = 100; // Default fallback
        }
        $paged = isset( $_GET['paged'] ) ? max( 1, intval( $_GET['paged'] ) ) : 1;

        // Build query args - IMPORTANT: Never exclude any orders assigned to vendor
        $args = [
            'limit' => $per_page,
            ...
            'paginate' => true,
            'post_status' => 'any', // Include all order statuses
            'type' => 'shop_order', // Ensure we get all WooCommerce orders
        ];

        // Add status filter (but never exclude orders)
        if ( $status_filter !== 'all' ) {
            // Only filter display, never exclude from total count
            $args['status'] = 'wc-' . $status_filter;
        } else {
            // Show ALL statuses when 'all' is selected
            $args['status'] = [ 'wc-pending', 'wc-processing', 'wc-on-hold', 'wc-completed', 'wc-cancelled', 'wc-refunded', 'wc-failed', 'wc-shipped' ];
        }

        // Enhanced search with type - HPOS Compatible (simplified for now)
        if ( ! empty( $search ) ) {
            if ( $search_type === 'customer_name' ) {
                // Simplified search - use regular text search for HPOS compatibility
                $args['s'] = $search;
        ...
        // Location filter - HPOS Compatible
        if ( ! empty( $customer_country ) ) {
            $args['_billing_country'] = $customer_country;

            if ( ! empty( $customer_state ) ) {
                $args['_billing_state'] = $customer_state;
            }
        }
        
        // Priority filter - HPOS Compatible
        if ( ! empty( $priority_filter ) ) {
            $args['_vss_order_priority'] = $priority_filter;
        }
        
        // Shipping status filter (late, on-time, etc.)
        if ( ! empty( $ship_status ) ) {
            ...
            foreach ( $all_orders as $order_id ) {
                ...
                if ( $ship_date ) {
                    ...
                    if ( ( $ship_status === 'late' && $is_late ) || ( $ship_status === 'on-time' && ! $is_late ) ) {
        ...
        <div class="vss-orders-header">
            <h2><?php esc_html_e( 'My Orders', 'vss' ); ?></h2>
            <div class="vss-bulk-actions">
                <button class="button vss-select-all"><?php esc_html_e( 'Select All', 'vss' ); ?></button>
                <select class="vss-bulk-action-select" disabled>
                    <option value=""><?php esc_html_e( 'Bulk Actions', 'vss' ); ?></option>
                    <option value="export"><?php esc_html_e( 'Export Selected', 'vss' ); ?></option>
                    <option value="mark-shipped"><?php esc_html_e( 'Mark as Shipped', 'vss' ); ?></option>
                    <option value="download-files"><?php esc_html_e( 'Download Files', 'vss' ); ?></option>
                </select>
                <button class="button button-primary vss-apply-bulk" disabled><?php esc_html_e( 'Apply', 'vss' ); ?></button>
                <button class="button vss-export-all" data-vendor="<?php echo esc_attr( $vendor_id ); ?>">
                    <span class="dashicons dashicons-download"></span>
                    <?php esc_html_e( 'Export All', 'vss' ); ?>
                </button>
            </div>
        </div>
        ...
        <div class="vss-filters-section">
            ...
            <div class="vss-filter-panel" style="display: none;">
                <form method="get" class="vss-filter-form">
                    ...
                    <div class="filter-group">
                        <h4><?php esc_html_e( 'Priority & Status', 'vss' ); ?></h4>
                        <div class="priority-inputs">
                            <select name="priority" class="vss-priority-select">
                                <option value=""><?php esc_html_e( 'All Priorities', 'vss' ); ?></option>
                                <option value="normal" <?php selected( $priority_filter, 'normal' ); ?>><?php esc_html_e( 'Normal', 'vss' ); ?></option>
                                <option value="high" <?php selected( $priority_filter, 'high' ); ?>><?php esc_html_e( 'High Priority', 'vss' ); ?></option>
                                <option value="urgent" <?php selected( $priority_filter, 'urgent' ); ?>><?php esc_html_e( 'Urgent', 'vss' ); ?></option>
                            </select>
                            
                            <select name="ship_status" class="vss-ship-status-select">
                                <option value=""><?php esc_html_e( 'All Shipping Status', 'vss' ); ?></option>
                                <option value="on-time" <?php selected( $ship_status, 'on-time' ); ?>><?php esc_html_e( 'On Time', 'vss' ); ?></option>
                                <option value="late" <?php selected( $ship_status, 'late' ); ?>><?php esc_html_e( 'Late', 'vss' ); ?></option>
                                <option value="no-date" <?php selected( $ship_status, 'no-date' ); ?>><?php esc_html_e( 'No Ship Date', 'vss' ); ?></option>
                            </select>
                        </div>
                    </div>
        ...
        <?php if ( $total_orders > 0 ) : ?>
            <div class="vss-orders-controls">
                <div class="vss-orders-info">
                    <?php
                    $start = ( ( $paged - 1 ) * $per_page ) + 1;
                    $end = min( $paged * $per_page, $total_orders );
                    
                    // Get lifetime total for this vendor
                    $lifetime_total_args = [
                        'meta_key' => '_vss_vendor_user_id',
                        'meta_value' => $vendor_id,
                        'return' => 'ids',
                    ...
                    $lifetime_total = count( wc_get_orders( $lifetime_total_args ) );
                    
                    printf(
                        esc_html__( 'Showing %1$d-%2$d of %3$d filtered orders (Total lifetime orders: %4$d)', 'vss' ),
                        $start,
                        $end,
                        $total_orders,
                        $lifetime_total
                ...
                <div class="vss-per-page-selector">
                    <label for="vss-per-page"><?php esc_html_e( 'Show:', 'vss' ); ?></label>
                    <select id="vss-per-page" class="vss-per-page-select">
                        <option value="25" <?php selected( $per_page, 25 ); ?>>25 <?php esc_html_e( 'per page', 'vss' ); ?></option>
                        <option value="50" <?php selected( $per_page, 50 ); ?>>50 <?php esc_html_e( 'per page', 'vss' ); ?></option>
                        <option value="100" <?php selected( $per_page, 100 ); ?>>100 <?php esc_html_e( 'per page', 'vss' ); ?></option>
                        <option value="250" <?php selected( $per_page, 250 ); ?>>250 <?php esc_html_e( 'per page', 'vss' ); ?></option>
                        <option value="500" <?php selected( $per_page, 500 ); ?>>500 <?php esc_html_e( 'per page', 'vss' ); ?></option>
                        <option value="1000" <?php selected( $per_page, 1000 ); ?>>1000 <?php esc_html_e( 'per page', 'vss' ); ?></option>
                    </select>
                </div>
            </div>
        <?php endif; ?>

        <table class="vss-orders-table enhanced-orders">
            <thead>
                <tr>
                    <th class="check-column">
                        <input type="checkbox" class="vss-select-all-checkbox">
                    </th>
            ...
            <tbody>
                    ...
                    ?>
                        <tr class="<?php echo esc_attr( $row_class ); ?>">
                            <td class="check-column">
                                <input type="checkbox" class="vss-order-checkbox" name="order_ids[]" value="<?php echo esc_attr( $order_id ); ?>">
                            </td>
                            <td>
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="8">
                            <div class="vss-no-orders-message">
                                <?php 
                                // Check if vendor has any lifetime orders
                                $lifetime_check = wc_get_orders( [
                                    'meta_key' => '_vss_vendor_user_id',
                                    'meta_value' => $vendor_id,
                                    'limit' => 1,
                                    'return' => 'ids',
                                ...
                                if ( ! empty( $lifetime_check ) ) {
                                    // Vendor has orders but current filters show none
                                    esc_html_e( 'No orders match your current filters. Try adjusting your search criteria or ', 'vss' );
                                    echo '<a href="' . esc_url( add_query_arg( 'vss_action', 'orders', get_permalink() ) ) . '">' . esc_html__( 'clear all filters', 'vss' ) . '</a>';
                                    echo ' ' . esc_html__( 'to see all your orders.', 'vss' );
                                } else {
                                    // Vendor has no orders yet
                                    esc_html_e( 'No orders have been assigned to you yet. Orders will appear here once they are assigned.', 'vss' );
                                }
                                ?>
                            </div>
                        </td>
        ...
        <?php if ( $total_pages > 1 ) : ?>
            <div class="vss-pagination">
                <?php
                // Build base URL preserving ALL query parameters
                $base_url = add_query_arg( [
                    'vss_action' => 'orders',
                    'status' => $status_filter !== 'all' ? $status_filter : false,
                    's' => ! empty( $search ) ? $search : false,
                    'search_type' => ! empty( $search_type ) && $search_type !== 'all' ? $search_type : false,
                    'date_from' => ! empty( $date_from ) ? $date_from : false,
                    'date_to' => ! empty( $date_to ) ? $date_to : false,
                    'customer_country' => ! empty( $customer_country ) ? $customer_country : false,
                    'customer_state' => ! empty( $customer_state ) ? $customer_state : false,
                    'priority' => ! empty( $priority_filter ) ? $priority_filter : false,
                    'ship_status' => ! empty( $ship_status ) ? $ship_status : false,
                    'per_page' => $per_page != 100 ? $per_page : false,
                ], get_permalink() );
        ...
        jQuery(document).ready(function($) {
            ...
            $('.vss-select-all, .vss-select-all-checkbox').on('change click', function(e) {
                if ($(this).is('button')) {
                    e.preventDefault();
                    var allChecked = $('.vss-order-checkbox:checked').length === $('.vss-order-checkbox').length;
                    $('.vss-order-checkbox, .vss-select-all-checkbox').prop('checked', !allChecked);
                    $(this).text(allChecked ? '<?php echo esc_js( __( 'Select All', 'vss' ) ); ?>' : '<?php echo esc_js( __( 'Deselect All', 'vss' ) ); ?>');
                } else {
                    $('.vss-order-checkbox').prop('checked', $(this).prop('checked'));
                }
                updateBulkActions();
            });
            
            // Individual checkbox change
            $(document).on('change', '.vss-order-checkbox', function() {
                updateBulkActions();
                var allChecked = $('.vss-order-checkbox:checked').length === $('.vss-order-checkbox').length;
                $('.vss-select-all-checkbox').prop('checked', allChecked);
            });
            ...
            function updateBulkActions() {
                ...
                $('.vss-bulk-action-select, .vss-apply-bulk').prop('disabled', disabled);
                
                if (checkedCount > 0) {
                    $('.vss-bulk-action-select').removeClass('disabled');
                    $('.vss-apply-bulk').removeClass('disabled');
                } else {
                    $('.vss-bulk-action-select').addClass('disabled');
                    $('.vss-apply-bulk').addClass('disabled');
                }
            }
            
            // Apply bulk action
            $('.vss-apply-bulk').on('click', function() {
                var action = $('.vss-bulk-action-select').val();
                if (!action) {
                    alert('<?php echo esc_js( __( 'Please select a bulk action.', 'vss' ) ); ?>');
                    return;
                }
            ...
            function exportSelectedOrders(orderIds) {
                var url = '<?php echo admin_url( 'admin-ajax.php' ); ?>?action=vss_export_orders';
                url += '&format=csv';
                url += '&order_ids=' + orderIds.join(',');
                url += '&nonce=<?php echo wp_create_nonce( 'vss_export_orders' ); ?>';
                window.location.href = url;
            }
            
            // Per page selector
            $('#vss-per-page').on('change', function() {
            ...
            // Export all orders (respecting current filters)
            $('.vss-export-all').on('click', function() {
                var vendorId = $(this).data('vendor');
                var confirmMsg = '<?php echo esc_js( __( 'This will export ALL orders matching current filters. Continue?', 'vss' ) ); ?>';
                ...
                url += '&status=<?php echo esc_js( $status_filter ); ?>';
                url += '&search=<?php echo esc_js( $search ); ?>';
                url += '&date_from=<?php echo esc_js( $date_from ); ?>';
                url += '&date_to=<?php echo esc_js( $date_to ); ?>';
                url += '&nonce=<?php echo wp_create_nonce( 'vss_export_orders' ); ?>';
                window.location.href = url;
            });
            
            // Advanced search suggestions
            var searchInput = $('.vss-search-input');
