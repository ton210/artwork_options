...
define( 'SWPD_VERSION', '12.1.7' ); // Incremented version
...
// Define modular mode (set to true to use ES6 modules, false for monolithic file)
// Temporarily disabled while fixing syntax issues in original file
define( 'SWPD_USE_MODULAR', false );
...
require_once SWPD_PLUGIN_DIR . 'includes/class-swpd-debug-page.php';
require_once SWPD_PLUGIN_DIR . 'includes/class-swpd-v2-debug.php';
...
function swpd_run_designer() {
    ...
    $swpd_size_selector = new SWPD_Size_Selector( $logger );
    $swpd_error_handler = new SWPD_Error_Handler( $logger );
    ...
    add_action( 'plugins_loaded', array( $swpd_size_selector, 'init' ) );
    add_action( 'plugins_loaded', array( $swpd_error_handler, 'init' ) );
...
add_action( 'wp_head', function() {
    if ( is_product() || is_shop() ) {
        ...
        /* SWPD Safety Patches & AJAX Configuration */

        // Global protection against document.body.classList errors
        (function() {
            'use strict';
            ...
            // Patch classList.contains to handle null document.body
            DOMTokenList.prototype.contains = function() {
                try {
                    return originalContains.apply(this, arguments);
                } catch (error) {
                    console.warn('SWPD: Prevented classList error:', error.message);
                    return false;
                }
            };

            // Patch document.body access
        ...
        window.swpd_ajax = window.swpd_ajax || {
            url: '<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>',
            nonce: '<?php echo esc_js( wp_create_nonce( 'swpd_design_upload_nonce' ) ); ?>',
            debug: <?php echo wp_doing_ajax() || ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? 'true' : 'false'; ?>
        };
        </script>
        <?php
    }
}, 1 ); // Run early to patch before other scripts
