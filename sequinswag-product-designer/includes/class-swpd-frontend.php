...
class SWPD_Frontend {
    ...
    public function enqueue_scripts() {
        ...
        // Create CSS cache buster version
        $css_cache_buster = SWPD_VERSION . '.' . time();

        // In development, add file modification time for CSS cache busting
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            $css_file_path = SWPD_PLUGIN_DIR . 'assets/css/designer-styles.css';
            if ( file_exists( $css_file_path ) ) {
                $css_cache_buster = SWPD_VERSION . '.' . filemtime( $css_file_path );
            }
        }
        ...
        // Create cache buster version
        $cache_buster = SWPD_VERSION . '.' . time();

        // In development, add file modification time for aggressive cache busting
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            $js_file_path = SWPD_PLUGIN_DIR . 'assets/js/enhanced-product-designer-working.js';
            if ( file_exists( $js_file_path ) ) {
                $cache_buster = SWPD_VERSION . '.' . filemtime( $js_file_path );
            }
        }

        // Choose between modular or monolithic version
        $use_modular = defined( 'SWPD_USE_MODULAR' ) && SWPD_USE_MODULAR;

        if ( $use_modular ) {
            // Enqueue modular version (ES6 modules)
            wp_enqueue_script(
                'swpd-enhanced-product-designer',
                SWPD_PLUGIN_URL . 'assets/js/modular/main.js',
            ...
            // Enqueue working version (syntax error free)
            wp_enqueue_script(
                'swpd-enhanced-product-designer',
                SWPD_PLUGIN_URL . 'assets/js/enhanced-product-designer-working.js',
                array( 'jquery', 'fabric-js', 'cropper-js' ),
                $cache_buster,
                true
            );
