...
const FRANCHISE_BRANDS = ['JARS', 'Exclusive', 'Cookies', 'House of Dank', 'Lume', 'Skymint', 'Cloud Cannabis', 'Gage'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, MI ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Michigan dispensary`
      : `${name} ${city} Michigan dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'MI')
...
async function importMichiganDispensaries() {
  console.log('Starting Michigan dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./michigan-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Michigan dispensaries\n`);
