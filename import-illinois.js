...
// Known franchise brands in Illinois
const FRANCHISE_BRANDS = [
  'Verilife', 'Consume Cannabis', 'Rise', 'Sunnyside', 'Zen Leaf', 'Curaleaf',
  'MedMen', 'The Herbal Care Center', 'Ascend', 'Columbia Care', 'Cresco',
  'Nature\'s Care', 'Bloc Dispensary', 'Cookies', 'Green Gate'
];
...
async function searchAndEnrichDispensary(dispensaryInfo) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address}, ${city}, IL ${zipCode}`);
  ...
  try {
    ...
    const searchQuery = `${name} ${address} ${city} Illinois dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties
       WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'IL')
...
async function importIllinoisDispensaries() {
  console.log('Starting Illinois dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./illinois-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Illinois dispensaries to process\n`);
