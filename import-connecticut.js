...
const FRANCHISE_BRANDS = ['Fine Fettle', 'Zen Leaf', 'Curaleaf', 'Shangri-La', 'INSA', 'The Botanist'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, CT ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Connecticut dispensary`
      : `${name} ${city} Connecticut dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'CT')
...
async function importConnecticutDispensaries() {
  console.log('Starting Connecticut dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./connecticut-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Connecticut dispensaries\n`);
