...
const FRANCHISE_BRANDS = ['Green Goods', 'Rise'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, MN ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Minnesota dispensary`
      : `${name} ${city} Minnesota dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'MN')
...
async function importMinnesotaDispensaries() {
  console.log('Starting Minnesota dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./minnesota-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Minnesota dispensaries\n`);
