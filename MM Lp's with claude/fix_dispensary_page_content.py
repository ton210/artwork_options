...
# Delete the empty page first
delete_url = f'{api_base_url}/content/pages/{page_id}'
delete_response = requests.delete(delete_url, headers=headers)
if delete_response.status_code in [204, 200]:
    print(f"✓ Deleted empty page {page_id}")
else:
    print(f"Could not delete page: {delete_response.status_code}")

# Now recreate with correct field name and content
html_content = '''<!-- MunchMakers Dispensary Landing Page - B2B Custom Cannabis Accessories -->
...
<div style="margin: 0 -20px; background: #f8f9fa; padding: 40px 20px;">
    <div style="max-width: 1000px; margin: 0 auto; text-align: center;">
        <img src="https://store-tqjrceegho.mybigcommerce.com/product_images/dispensary_hero.png" alt="Premium dispensary display with custom accessories" style="width: 100%; max-width: 800px; height: auto; border-radius: 12px; box-shadow: 0 20px 60px rgba(0,0,0,0.15);">
    </div>
</div>
...
<div id="calculator" style="background: linear-gradient(135deg, #f5f5f5, #ffffff); padding: 60px 20px;">
    <div style="max-width: 900px; margin: 0 auto;">
        ...
        <div style="text-align: center; margin-top: 30px;">
            <img src="https://store-tqjrceegho.mybigcommerce.com/product_images/dispensary_profit.png" alt="Profit visualization" style="max-width: 500px; width: 100%; height: auto; border-radius: 10px;">
        </div>
    </div>
</div>
...
<div style="padding: 60px 20px; background: white;">
    <div style="max-width: 1200px; margin: 0 auto;">
        ...
        <div style="text-align: center; margin-top: 50px;">
            <img src="https://store-tqjrceegho.mybigcommerce.com/product_images/dispensary_customer.png" alt="Happy customer with custom accessories" style="max-width: 700px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
        </div>
    </div>
</div>
...
# Create the page - try different field names
field_names_to_try = ['body', 'content', 'html_body', 'html', 'description']

for field_name in field_names_to_try:
    print(f"\nTrying to create page with field name: '{field_name}'")

    page_data = {
        "type": "page",
        "name": "Custom Cannabis Accessories for Dispensaries",
        field_name: html_content,  # Try different field names
        "is_visible": True,
        "parent_id": 0,
        "sort_order": 100,
        "meta_description": "Increase dispensary profits by 73% with custom grinders & accessories. No minimums, 5-day production.",
        "search_keywords": "dispensary accessories, custom grinders wholesale"
    }
    ...
    if response.status_code == 201:
        ...
        print(f"\n✓✓✓ PAGE CREATED SUCCESSFULLY WITH FIELD '{field_name}'! ✓✓✓")
        print(f"\nPage Details:")
        ...
        print(f"  Failed with {field_name}: {response.status_code} - {response.json().get('detail', 'Unknown error')}")
