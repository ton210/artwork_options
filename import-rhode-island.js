...
const FRANCHISE_BRANDS = ['Rise', 'Solar Cannabis'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, RI ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Rhode Island dispensary`
      : `${name} ${city} Rhode Island dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'RI')
...
async function importRhodeislandDispensaries() {
  console.log('Starting Rhode Island dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./rhode-island-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Rhode Island dispensaries\n`);
