...
// Known franchise brands in Missouri
...
async function searchAndEnrichDispensary(dispensaryInfo) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address}, ${city}, MO ${zipCode}`);
  ...
  try {
    ...
    const searchQuery = `${name} ${address} ${city} Missouri dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties
       WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'MO')
...
async function importMissouriDispensaries() {
  console.log('Starting Missouri dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./missouri-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Missouri dispensaries to process\n`);
