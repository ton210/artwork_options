...
const FRANCHISE_BRANDS = ['Bloom', 'The Forest', 'Rise', 'The Botanist', 'Terrasana', 'Story Cannabis', 'Zen Leaf'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, OH ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Ohio dispensary`
      : `${name} ${city} Ohio dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'OH')
...
async function importOhioDispensaries() {
  console.log('Starting Ohio dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./ohio-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Ohio dispensaries\n`);
