...
const FRANCHISE_BRANDS = ['Have a Heart', 'Dockside', "Uncle Ike's", 'Clutch', 'Higher Leaf', 'Zips Cannabis', 'Evergreen Market', 'Main Street Marijuana'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, WA ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Washington dispensary`
      : `${name} ${city} Washington dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'WA')
...
async function importWashingtonDispensaries() {
  console.log('Starting Washington dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./washington-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Washington dispensaries\n`);
