...
async function searchAndEnrichDispensary(dispensaryInfo) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address}, ${city}, MA ${zipCode}`);
  ...
  try {
    ...
    const searchQuery = `${name} ${address} ${city} Massachusetts dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties
       WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'MA')
...
async function importMassachusettsDispensaries() {
  console.log('Starting Massachusetts dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./massachusetts-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Massachusetts dispensaries to process\n`);
...
importMassachusettsDispensaries();
