...
const FRANCHISE_BRANDS = [
  'Zen Leaf', 'Curaleaf', 'Rise', 'Ascend', 'The Apothecarium', 'Cannabist',
  'Ayr Wellness', 'Breakwater', 'Columbia Care', 'Harmony', 'TerrAscend',
  'Verano', 'GTI', 'Green Thumb Industries'
];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, NJ ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} New Jersey dispensary`
      : `${name} ${city} New Jersey dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'NJ')
...
async function importNewJerseyDispensaries() {
  console.log('Starting New Jersey dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./new-jersey-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} NJ dispensaries\n`);
