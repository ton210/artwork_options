...
const FRANCHISE_BRANDS = ['Theory Wellness'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, ME ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Maine dispensary`
      : `${name} ${city} Maine dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'ME')
...
async function importMaineDispensaries() {
  console.log('Starting Maine dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./maine-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Maine dispensaries\n`);
