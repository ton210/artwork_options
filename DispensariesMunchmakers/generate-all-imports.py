#!/usr/bin/env python3
"""Generate import scripts for all remaining states"""

STATES = {
    'nevada': {'abbr': 'NV', 'brands': ['Rise', 'Thrive', 'Curaleaf', 'The Dispensary', 'Deep Roots Harvest', 'The Source', 'Zen Leaf', 'Green', 'Planet 13', 'Oasis', 'Jardin', 'Cookies']},
    'oregon': {'abbr': 'OR', 'brands': ['Nectar', 'Electric Lettuce', 'Serra', 'Chalice', 'Oregrown', 'La Mota', 'Substance Market', 'Cannabis Nation', 'Lucky Lion']},
    'colorado': {'abbr': 'CO', 'brands': ['The Green Solution', 'Native Roots', 'LivWell', 'Star Buds', 'Green Dragon', 'Lightshade', 'Medicine Man', 'Good Chemistry']},
    'washington': {'abbr': 'WA', 'brands': ['Have a Heart', 'Dockside', 'Uncle Ike\'s', 'Clutch', 'Higher Leaf', 'Zips Cannabis', 'Evergreen Market', 'Main Street Marijuana']},
    'michigan': {'abbr': 'MI', 'brands': ['JARS', 'Exclusive', 'Cookies', 'House of Dank', 'Lume', 'Skymint', 'Cloud Cannabis', 'Gage']},
    'california': {'abbr': 'CA', 'brands': ['Cookies', 'Jungle Boys', 'MedMen', 'STIIIZY', 'Catalyst', 'March and Ash', 'Urbn Leaf', 'Harborside', 'The Pottery']},
    'maine': {'abbr': 'ME', 'brands': ['Theory Wellness']},
    'connecticut': {'abbr': 'CT', 'brands': ['Fine Fettle', 'Zen Leaf', 'Curaleaf', 'Shangri-La', 'INSA', 'The Botanist']},
    'new-york': {'abbr': 'NY', 'brands': ['Housing Works', 'The Travel Agency', 'MedMen', 'Curaleaf', 'The Botanist', 'Verilife']},
    'rhode-island': {'abbr': 'RI', 'brands': ['Rise', 'Solar Cannabis']},
    'alaska': {'abbr': 'AK', 'brands': []},
    'delaware': {'abbr': 'DE', 'brands': ['Curaleaf', 'First State Compassion']},
    'maryland': {'abbr': 'MD', 'brands': ['Zen Leaf', 'Culta', 'Verilife', 'Green Goods', 'Liberty', 'Rise', 'Curaleaf']},
    'minnesota': {'abbr': 'MN', 'brands': ['Green Goods', 'Rise']},
    'montana': {'abbr': 'MT', 'brands': ['Bloom', 'Elevated']},
    'new-mexico': {'abbr': 'NM', 'brands': ['Ultra Health', 'Pecos Valley', 'Verdes', 'R Greenleaf', 'Sandia Botanicals', 'Urban Wellness']},
    'ohio': {'abbr': 'OH', 'brands': ['Bloom', 'The Forest', 'Rise', 'The Botanist', 'Terrasana', 'Story Cannabis', 'Zen Leaf']},
    'washington-dc': {'abbr': 'DC', 'brands': ['District Cannabis', 'Cookies']}
}
...
async function searchAndEnrichDispensary(info) {{
  ...
  console.log(`\\n=== Processing: ${{name}} ===`);
  console.log(`Address: ${{address || 'N/A'}}, ${{city}}, {abbr} ${{zipCode || ''}}`);
  ...
  try {{
    ...
    const searchQuery = address
      ? `${{name}} ${{address}} ${{city}} {state_name} dispensary`
      : `${{name}} ${{city}} {state_name} dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = '{abbr}')
...
async function import{state_func}Dispensaries() {{
  console.log('Starting {state_name} dispensaries import...\\n');

  try {{
    const csv = fs.readFileSync('./{state_file}-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${{data.length}} {state_name} dispensaries\\n`);
...
import{state_func}Dispensaries();
'''

for state, config in STATES.items():
    state_name = state.replace('-', ' ').title()
    state_func = state.replace('-', '').title()
    state_file = state
    abbr = config['abbr']
    brands_str = str(config['brands'])
    ...
    filename = f'import-{state}.js'
    with open(filename, 'w') as f:
        f.write(content)
    print(f'✓ Created {filename}')

print(f'\n✅ Generated {len(STATES)} import scripts!')
