...
const FRANCHISE_BRANDS = ['Housing Works', 'The Travel Agency', 'MedMen', 'Curaleaf', 'The Botanist', 'Verilife'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, NY ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} New York dispensary`
      : `${name} ${city} New York dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'NY')
...
async function importNewyorkDispensaries() {
  console.log('Starting New York dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./new-york-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} New York dispensaries\n`);
