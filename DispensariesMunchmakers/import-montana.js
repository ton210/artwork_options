...
const FRANCHISE_BRANDS = ['Bloom', 'Elevated'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, MT ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Montana dispensary`
      : `${name} ${city} Montana dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'MT')
...
async function importMontanaDispensaries() {
  console.log('Starting Montana dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./montana-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Montana dispensaries\n`);
