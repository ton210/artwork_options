...
const FRANCHISE_BRANDS = ['Ultra Health', 'Pecos Valley', 'Verdes', 'R Greenleaf', 'Sandia Botanicals', 'Urban Wellness'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, NM ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} New Mexico dispensary`
      : `${name} ${city} New Mexico dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'NM')
...
async function importNewmexicoDispensaries() {
  console.log('Starting New Mexico dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./new-mexico-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} New Mexico dispensaries\n`);
