...
async function searchAndEnrichDispensary(dispensaryInfo) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address}, ${city}, VT ${zipCode}`);
  ...
  try {
    ...
    const searchQuery = `${name} ${address} ${city} Vermont dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties
       WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'VT')
...
async function importVermontDispensaries() {
  console.log('Starting Vermont dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./vermont-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Vermont dispensaries to process\n`);
