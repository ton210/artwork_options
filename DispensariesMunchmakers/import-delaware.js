...
const FRANCHISE_BRANDS = ['Curaleaf', 'First State Compassion'];
...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, DE ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Delaware dispensary`
      : `${name} ${city} Delaware dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'DE')
...
async function importDelawareDispensaries() {
  console.log('Starting Delaware dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./delaware-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Delaware dispensaries\n`);
