...
async function searchAndEnrichDispensary(info) {
  ...
  console.log(`\n=== Processing: ${name} ===`);
  console.log(`Address: ${address || 'N/A'}, ${city}, AK ${zipCode || ''}`);
  ...
  try {
    ...
    const searchQuery = address
      ? `${name} ${address} ${city} Alaska dispensary`
      : `${name} ${city} Alaska dispensary`;
    ...
    const countyResult = await db.query(
      `SELECT id FROM counties WHERE state_id = (SELECT id FROM states WHERE abbreviation = 'AK')
...
async function importAlaskaDispensaries() {
  console.log('Starting Alaska dispensaries import...\n');

  try {
    const csv = fs.readFileSync('./alaska-dispensaries.csv', 'utf8');
    ...
    console.log(`Found ${data.length} Alaska dispensaries\n`);
